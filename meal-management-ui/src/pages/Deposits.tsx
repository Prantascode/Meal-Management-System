  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [mRes, dRes] = await Promise.all([
        api.get('/members'),
        api.get(
          `/deposit/total/range?startDate=${firstDay}&endDate=${lastDay}`
        )
      ]);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 font-medium">
            Track and manage mess fund contributions
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {Object.entries(memberTotals).map(([name, total]: any) => (
              <div
                key={name}
                className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 hover:border-indigo-200 transition-colors"
              >
                ...
                <div>
                  <p className="text-slate-400 text-[10px] font-black uppercase truncate max-w-[120px]">
                    {name}
                  </p>
      ...
      {isAdding && isAdmin && (
        <div className="max-w-md bg-white p-8 rounded-3xl shadow-2xl border border-slate-100 mx-auto animate-in slide-in-from-bottom-4 duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 uppercase tracking-widest text-xs"
            >
              Save Transaction
            </button>
          </form>
